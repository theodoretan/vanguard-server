...
characterSchema.statics.updateUserCharacter = function(username, c1, c2, c3) {
	return new Promise((resolve, reject) => {
		this.model('Character').findOne({ 'username': username }, (err, character) => {
			if (err) reject(err);
			character.character1 = c1;
			character.character2 = c2;
			character.character3 = c3;

			character.save((err, updated) => {
				if (err) reject(err);
				resolve(updated);
			});
		});
...
characterSchema.statics.getUserCharacter = function(username) {
	return new Promise((resolve, reject) => {
